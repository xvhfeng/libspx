...
#include "spx_atomic.h"
	...
	__SpxPrivate__ spx_atomic_t u64_t vfreelock = 0;
...
#define SPX_KLIST_HEAD(name) \
	...
	__SpxPrivate__ __SpxInline__ spx_ret_t __spx_kfreelist_add(spx_klist_node_t *new,
			spx_klist_node_t *prev,
			spx_klist_node_t *next) {
		spx_atomic_t u64_t version = vfreelock;
		new->next = next;
		new->prev = prev;
		if (spx_atomic_iscas(prev->next, next, new) 
				&& spx_atomic_iscas(next->prev, prev, new)
				&& spx_atomic_iscas(vfreelock,version, version + 1)) {
			spx_atomic_mb();
			return SpxRet_OK;
		}
		new->next = NULL;
		new->prev = NULL;
		return SpxRet_FREELOCK;
	}
	 ...
	 */
	__SpxPrivate__ __SpxInline__ void spx_kfreelist_add(spx_klist_node_t *new, spx_klist_node_t *head) {
		spx_ret_t ret = SpxRet_OK;
		spx_atomic_t u64_t version = vfreelock;
		spx_klist_node_t *prev = NULL;
		spx_klist_node_t *next = NULL;
		prev = head;
		next = head->next;
		while(true) {
			if(prev->next != next) {
			...
			new->next = next;
			new->prev = prev;
			if((spx_atomic_iscas(vfreelock,version,version + 1))
					&& spx_atomic_iscas(prev->next,next,new)) {
				spx_atomic_iscas(next->prev, prev, new);
				break;
			}
		}
	 ...
	 */
	__SpxPrivate__ __SpxInline__ void spx_kfreelist_add_tail(spx_klist_node_t *new, spx_klist_node_t *head) {
		spx_ret_t ret = SpxRet_OK;
		do {    
			ret = __spx_kfreelist_add(new, head->prev, head);
		}while(SpxRet_OK != ret);
	}


	__SpxPrivate__ __SpxInline__ spx_ret_t __spx_kfreelist_del_entry(spx_klist_node_t *entry,
			spx_klist_node_t *prev,
			spx_klist_node_t *next) {
		spx_atomic_t u64_t version = vfreelock;    
		if (spx_atomic_iscas_p(&(prev->next), entry, next) 
				&& spx_atomic_iscas_p(&(next->prev), entry, prev)
				&& ((version == vfreelock) && (++vfreelock))) {
			return SpxRet_OK;
		}
		return SpxRet_FREELOCK;
	}
	...
	__SpxPrivate__ __SpxInline__ void spx_kfreelist_del(spx_klist_node_t *entry) {
		spx_ret_t ret = SpxRet_OK;
		do {
			ret = __spx_kfreelist_del_entry(entry,entry->prev, entry->next);
		}while(SpxRet_OK != ret);
		entry->next = SPX_KLIST_POISON1;
		entry->prev = SPX_KLIST_POISON2;
	}

	__SpxPrivate__ __SpxInline__ spx_klist_node_t *spx_kfreelist_del_tail(spx_klist_node_t *head) {
		spx_klist_node_t *entry = NULL;
		spx_ret_t ret = SpxRet_OK;
		do {
			entry = head->prev;
			ret = __spx_kfreelist_del_entry(entry,entry->prev,entry->next);
	...
	__SpxPrivate__ __SpxInline__ spx_klist_node_t *spx_kfreelist_del_head(spx_klist_node_t *head) {
		spx_klist_node_t *entry = NULL;
		spx_ret_t ret = SpxRet_OK;
		do {
			entry = head->next;
